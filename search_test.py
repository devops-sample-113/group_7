...
def update_keywords():
    global professor_keyword, course_keyword  # 讓這兩個變數可以在函數外部使用並更新
    professor_keyword = professor_entry.get().strip()  # 取得教授名稱關鍵字
    course_keyword = course_entry.get().strip()  # 取得課程名稱關鍵字
    display_courses()

Label(Swindow, text="教授名稱：").place(x=10, y=10)
professor_entry = Entry(Swindow, width=20)
professor_entry.place(x=100, y=10)

Label(Swindow, text="課程名稱：").place(x=10, y=40)
course_entry = Entry(Swindow, width=20)
course_entry.place(x=100, y=40)

Button(Swindow, text="搜尋", command=update_keywords).place(x=280, y=25)
...
def display_courses():
    ...
    for row_idx, row in enumerate(worksheet_courses.iter_rows(min_row=2, max_row=53, min_col=1, max_col=5, values_only=True), start=1):
        ...
        if professor_keyword in professor_name and course_keyword in course_name:
